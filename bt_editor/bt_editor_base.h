...
struct NodeModel {
  ...
  bool operator!=(const NodeModel &other) const { return !(*this == other); }
...
struct AbstractTreeNode {
  AbstractTreeNode()
      : index(-1), status(NodeStatus::IDLE), graphic_node(nullptr) {
...
public:
  ...
  size_t nodesCount() const { return _nodes.size(); }
  ...
  AbstractTreeNode *addNode(AbstractTreeNode *parent,
