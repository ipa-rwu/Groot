...
void AbsBehaviorTree::clear() { _nodes.resize(0); }
...
std::vector<const AbstractTreeNode *>
AbsBehaviorTree::findNodes(const QString &instance_name) {
...
const AbstractTreeNode *
AbsBehaviorTree::findFirstNode(const QString &instance_name) {
  for (const auto &node : _nodes) {
    if (node.instance_name == instance_name) {
      return (&node);
    }
...
void AbsBehaviorTree::debugPrint() const {
  ...
  recursiveStep = [&](const AbstractTreeNode *node, int indent) {
    ...
    printf("%s (%s)", node->instance_name.toStdString().c_str(),
...
const char *toStr(GraphicMode type) {
  if (type == GraphicMode::EDITOR)
    return "EDITOR";
  if (type == GraphicMode::MONITOR)
    return "MONITOR";
  if (type == GraphicMode::REPLAY)
    return "REPLAY";
  return nullptr;
}
...
bool NodeModel::operator==(const NodeModel &other) const {
  bool is_same = (type == other.type && ports.size() == other.ports.size() &&
...
NodeModel &NodeModel::operator=(const BT::TreeNodeManifest &src) {
  ...
  for (const auto &port_it : src.ports) {
    ...
    port_model = bt_port;
    this->ports.insert(
        {QString::fromStdString(port_name), std::move(port_model)});
  }
  return *this;
}
