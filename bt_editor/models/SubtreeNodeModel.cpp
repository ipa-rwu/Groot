...
SubtreeNodeModel::SubtreeNodeModel(const NodeModel &model)
    : BehaviorTreeDataModel(model), _expanded(false) {
  _line_edit_name->setReadOnly(true);
  _line_edit_name->setHidden(true);

  _expand_button =
      new QPushButton(_expanded ? "Collapse" : "Expand", _main_widget);
  ...
  connect(_expand_button, &QPushButton::clicked, this,
          [this]() { emit expandButtonPushed(); });
