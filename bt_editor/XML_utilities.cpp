...
NodeModel buildTreeNodeModelFromXML(const QDomElement &node) {
  ...
  const auto node_type =
      BT::convertFromString<BT::NodeType>(tag_name.toStdString());
...
NodeModels ReadTreeNodesModel(const QDomElement &root) {
  ...
  if (!model_root.isNull()) {
    for (QDomElement node = model_root.firstChildElement(); !node.isNull();
...
void RecursivelyCreateXml(const FlowScene &scene, QDomDocument &doc,
                          QDomElement &parent_element, const Node *node) {
  ...
  if (BuiltinNodeModels().count(registration_name) != 0) {
    ...
    element =
        doc.createElement(QString::fromStdString(toStr(bt_node->nodeType())));
...
bool VerifyXML(QDomDocument &doc, const std::vector<QString> &registered_ID,
...
QDomElement writePortModel(const QString &port_name, const PortModel &port,
                           QDomDocument &doc) {
